...
- **Idempotency (Exactly-Once Semantics)**: High-performance databases drop `UNIQUE` constraints to speed up writes. The Analytics Service employs a Redis-based deduplication window (e.g., 24-hour TTL on `event_id`) paired with Kafka Streams / ksqlDB upstream for strict Exactly-Once streaming. This guarantees complete accuracy for critical metrics without sacrificing insert throughput.
